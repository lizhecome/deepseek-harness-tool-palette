...
function scoreTool(schema: ToolSchema, query: string): number {
  ...
  if (parameters.includes(query)) score += 120
  for (const token of tokens) {
    if (name.split(' ').includes(token)) score += 160
    else if (name.includes(token)) score += 100
    if (description.includes(token)) score += 35
    if (parameters.includes(token)) score += 15
  }
  return score
}
...
function searchableCatalog(ctx: Context): ToolSchema[] {
...
function validateBaseline(ctx: Context, config: ResolvedConfig): void {
...
function refreshRestriction(ctx: Context, state: PaletteState, config: ResolvedConfig): void {
  const known = new Set(ctx.tools.schemas().map(schema => schema.name))
  const allow = [
    TOOL_SEARCH_NAME,
...
function lineage(state: PaletteState): PaletteState[] {
  const result: PaletteState[] = []
  const seen = new Set<PaletteState>()
  let current: PaletteState | undefined = state
  while (current !== undefined && !seen.has(current)) {
    seen.add(current)
    if (!current.closed) result.unshift(current)
...
function installAgent(
  ...
  states: WeakMap<Agent, PaletteState>,
  liveStates: Set<PaletteState>,
): void {
...
export function apply(ctx: Context, config: Config): void {
  ...
  const states = new WeakMap<Agent, PaletteState>()
  const liveStates = new Set<PaletteState>()
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        const lines = value.matches.map(match =>
          `- ${match.name}${match.newlyUnlocked ? ' [unlocked]' : match.enabled ? ' [visible]' : ''}: ${match.description}`,
        )
        return [{
          type: 'text',
          text: [
    ...
    execute(args, exec) {
      if (exec.agent === undefined) throw new Error('tool_search requires a live Agent')
      const state = states.get(exec.agent)
      if (state === undefined || state.closed) throw new Error('tool_search is unavailable outside a managed live Agent')
      ...
      const matches: RankedTool[] = catalog
      ...
      if (shouldUnlock && selectedHidden.length > 0) {
        for (const member of lineage(state)) {
          for (const toolName of selectedHidden) member.unlocked.add(toolName)
    ...
    presentResult(args, result: ToolResult): GenericResultView {
      return {
        card: 'generic',
  ...
  ctx.effect(() => {
    let stopping = false
    for (const agent of ctx.agents.list()) installAgent(ctx, agent, resolved, states, liveStates)
    const stopCreated = ctx.on('agent/created', ({ agent }) => {
      if (!stopping) installAgent(ctx, agent, resolved, states, liveStates)
    })
    return () => {
      stopping = true
      stopCreated()
      for (const state of [...liveStates]) state.dispose?.()
    }
